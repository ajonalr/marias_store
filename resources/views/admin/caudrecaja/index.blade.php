            ...
            <div class="modal-dialog" role="document">
               <div class="modal-content">
                  <div class="modal-header">
                     <h5 class="modal-title">REGISTRO DE CUADRE DE CAJA</h5>
                     <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                     </button>
                  </div>
                  <form action="{{route('cuac.store')}}" method="post">
                     @csrf
                     @method('POST')
                  ...
                  <table class="table">
                     ...
                     <tbody>
                        ...
                        <tr>
                           ...
                           <td>{{$d->created_at}}</td>
                           <td>
                              <form action="{{route('cuac.destroy', ['id' => $d->id])}}" method="post">
                                 @csrf
                                 @method('DELETE')
                                 <a class="btn btn-primary" href="{{route('cuac.show', ['id' => $d->id])}}" method="post"><i class="fas fa-edit    "></i></a>
